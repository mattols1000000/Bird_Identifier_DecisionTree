...
async function testEBird(eyr: number) {
  // Use existing cookie from environment or login
  const ebirdUser = process.env.EBIRD_USER || "testuser";
  const ebirdPass = process.env.EBIRD_PASS || "testpass";
  ...
  try {
    ...
    console.log(`Success for eyr=${eyr}, data length:`, res.data.length);
  } catch (e: any) {
